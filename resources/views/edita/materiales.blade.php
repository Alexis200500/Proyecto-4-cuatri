...
<head>
    <meta charset="utf-8">
    {{Form::open(['route' => 'editarmateriales','files' => true])}}
    {{Form::token()}}
...
<body>
  <div align='center'>
      <h1><b>Modificar Material</b></h1>
      <table>
        <tr>
          <td>ID:</td>
          <td>
                   @if($errors->first('idm'))<p><i> {{ $errors->first('idm') }}</i></p> @endif
                  {{Form::text('idm',$consulta->idm,['readonly', 'class' => 'form-control'])}}</td>
        </tr>

        <tr>
          <td>Material:</td>
          <td>     @if($errors->first('material')) <p><i> {{ $errors->first('material') }}</i></p>@endif
              {{Form::text('material',$consulta->material,['class' => 'form-control'])}}</td>
        </tr>
