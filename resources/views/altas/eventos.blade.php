...
<head>
    ...
    <h1>Alta eventos</h1>
			...
			@if($errors->first('idev'))
			<p><i> {{ $errors->first('idev') }}</i></p>
			@endif
      ...
      <table>
        ...
        <tr>
          <td>Cliente:</td>
          <td><select class="form-control btn btn-primary" name = 'idcl'>
   		            @foreach($clie as $est)
   		              <option value = '{{$est->idcl}}'>{{$est->nombre}}</option>
   		            @endforeach
   		           </select></td>
        </tr>

        <tr>
          <td>Selecciona el Evento:</td>
          <td><select name ='idte' class="form-control btn btn-primary">
    	                           @foreach($tip as $est)
    	                            <option value = '{{$est->idte}}'>{{$est->evento}}</option>
    	                          @endforeach
    	                         </select></td>
        </tr>
        ...
        <tr>
          ...
          <td>{{Form::date('fechaevento', \Carbon\Carbon::now())}}</td>
        </tr>
        ...
        <tr>
          ...
          <td>@if($errors->first('cantpersona'))<p><i> {{ $errors->first('cantpersona') }} </i></p>@endif
        		{{Form::text('cantpersona', old('cantpersona'), ['class' => 'form-control','placeholder' => 'Cantidad de personas'])}}</td>
        </tr>

        <tr>
          <td>Admin:</td>
          <td><select name = 'idad' class="btn btn-info dropdown-toggle">
                  @foreach($ad as $est)
                    <option value = '{{$est->idad}}'>{{$est->nombre}}</option>
                  @endforeach
                </select></td>
        </tr>
